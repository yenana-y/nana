<template>
  ...
  <div class="header">
    <div class="box">
      ...
      <div class="find">
        <img
          ...
          alt
        />
        <router-link tag="input" to="/search" type="text" value="商品名称 作者 出版社 ISBN"></router-link>
      </div>
    </div>
    <!-- 是轮播图啊 -->
    <div class="pic">
    ...
    <div class="data" >
      <div class="data_box" v-for="(item,index) in firstList" :key="index">
        <div class="book">
          <p>{{item.title}}</p>
          <p>{{item.subTitle}}</p>
        </div>
    ...
    <div class="data" >
      <div class="data_box" v-for="(item,index) in twoList" :key="index">
        <div class="book">
          <p>{{item.title}}</p>
          <p>{{item.subTitle}}</p>
        </div>
    ...
    <div class="huan">换一换
        <img src="data:image/png;base64," alt="">
    </div>
    <!-- <div class="null"></div> -->
    <div class="auction">
        <span>销售排行</span>
...
export default {
  ...
  methods: {
    async handleGetFirstList(currPage) {
      ...
      async handleGetThindList(currPage) {
       let data = await firstApi(currPage);
      ...
      async handleGetFourList(currPage) {
       let data = await firstApi(currPage);
      ...
      async handleGetMaxList(currPage) {
       let data = await firstApi(currPage);
      ...
      async handleGetXiaoList(currPage) {
       let data = await firstApi(currPage);
...
.kuang .price span:first-child {
  font-size: 0.12rem;
  color: #9e100e;
}
.kuang .price span:nth-child(2) {
  font-size: 0.15rem;
  color: #9e100e;
}
.kuang .price span:nth-child(3) {
  font-size: 0.12rem;
  color: #999;
  margin-left: 0.02rem;
}
...
.newSquareBox_list .inner {
  width: 3.45rem;
  height: 4.5rem;
  margin: 0 auto;
}
.newSquareBox_list .inner li {
  width: 3.45rem;
  height: 1.5rem;
  position: relative;
  display: flex;
}
.newSquareBox_list .inner li .i {
  width: 1rem;
  height: 1.4rem;
  margin-right: 0.3rem;
}
...
.newSquareBox_list .inner li .i .l3 {
  width: 1rem;
  height: 1.4rem;
  padding-top: 0;
}
.newSquareBox_list .inner li .price {
  display: flex;
  ...
  color: #fff;
  padding: 0 0.07rem 0 0.02rem;
  border-radius: 0 0.35rem 0.35rem 0;
}
.newSquareBox_list .inner li .r {
  width: 2.3rem;
  height: 1.5rem;
  border-bottom: 1px solid #f3f3f3;
  overflow: hidden;
}
.newSquareBox_list .inner li .r .title {
  width: 100%;
  height: 0.21rem;
  line-height: 0.21rem;
  font-size: 0.15rem;
...
.newSquareBox_list .inner li .r .desc {
  width: 100%;
  height: 0.36rem;
  font-size: 0.12rem;
  color: #999;
...
.newSquareBox_list .inner li .r .author {
  color: #999;
  text-align: justify;
}
.newSquareBox_list .inner li .r .sign_box {
  margin-top: 0.2rem;
  font-size: 0;
  height: 0.22rem;
  overflow: hidden;
}
.newSquareBox_list .inner li .r .sign_box span {
  position: relative;
...
.newSquareBox_list .inner li .r .sign_box span .jiao {
  width: 0;
  height: 0;
  border-bottom: 0.16rem solid rgba(153, 127, 85, 0.6);
  border-left: 0.16rem solid transparent;
  position: absolute;
  right: -1px;
  bottom: -1px;
}
.newSquareBox_list .inner li .r .sign_box span .jiao .jiao_inner {
  width: 0;
  height: 0;
